...
describe("useProjects", () => {
  ...
  it("should return original English projects when language is English", () => {
    ...
    expect(result.current.projects[0].description).toEqual([
  ...
  it("should return translated German projects when language is German", () => {
    ...
    const aiPlaygroundProject = result.current.projects.find(
      (p) => p.title === "AI Playground"
    );
    ...
    const workshopProject = result.current.projects.find((p) =>
      p.title.includes("KI-Workshop")
    );
    ...
    const regularProject = result.current.projects.find(
      (p) => p.title === "Regular Project"
    );
    expect(regularProject).toBeDefined();
    expect(regularProject?.description).toEqual([
  ...
  it("should return same number of projects regardless of language", () => {
    ...
    expect(englishResult.current.projects.length).toBe(
      germanResult.current.projects.length
    );
  ...
  it("should update projects when language changes", () => {
    ...
    expect(aiPlaygroundProject?.description[0]).toContain(
      "ChatYourData, ein innovatives Technologieunternehmen"
    );
    expect(aiPlaygroundProject?.description[0]).toContain('**"Playground"**');
  });
  ...
  it("should handle partial translations correctly", () => {
    ...
    expect(workshopProject).toBeDefined();
    expect(workshopProject?.title).toBe("KI-Workshop für Steuersoftware");
    expect(workshopProject?.description[0]).toContain(
      "WIADOK, ein Unternehmen, das sich auf innovative Lösungen für Unternehmen, insbesondere Steuerberater, spezialisiert hat"
    );

    // AI Playground has only description translated, title stays the same
    const aiProject = result.current.projects.find(
      (p) => p.title === "AI Playground"
    );
    expect(aiProject?.title).toBe("AI Playground"); // Original title
    expect(aiProject?.description[0]).toContain(
      "ChatYourData, ein innovatives Technologieunternehmen"
    ); // German description
    expect(aiProject?.description[0]).toContain('**"Playground"**'); // Contains markdown formatting
  });
});
