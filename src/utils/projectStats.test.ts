...
describe("projectStats", () => {
  describe("calculateProjectStats", () => {
    ...
    it("should calculate primary tag statistics", () => {
      ...
      const reactPrimary = stats.primaryTagStats.find(
        (t) => t.name === "React"
      );
    ...
    it("should sort technologies by frequency", () => {
      ...
      expect(stats.topTechnologies[0].count).toBeGreaterThanOrEqual(
        stats.topTechnologies[1].count
      );
      expect(stats.topTechnologies[1].count).toBeGreaterThanOrEqual(
        stats.topTechnologies[2].count
      );
    });
