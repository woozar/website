...
describe('FlippableStatCard', () => {
  ...
  it('should update accessibility label when flipped', () => {
    ...
    const flipButton = screen.getByRole('button');
    expect(flipButton).toHaveAttribute('aria-label', 'Test Frameworks card. Click to show details.');
    
    fireEvent.click(flipButton);
    expect(flipButton).toHaveAttribute('aria-label', 'Test Frameworks card. Click to hide details.');
  });
  ...
  it('should render correct number of framework items', () => {
    ...
    const frameworkItems = screen.getAllByText(/React|Angular|Vue|Next\.js|Express/);
  ...
  it('should handle long framework names without wrapping', () => {
    ...
    expect(screen.getByText('Very Long Framework Name That Should Not Wrap')).toBeInTheDocument();
  });
});