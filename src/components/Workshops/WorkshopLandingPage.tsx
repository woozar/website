import { Stack, Title, Text, Card, Button, Box, List, ThemeIcon, Image } from '@mantine/core';
import { IconCheck, IconClock, IconUsers, IconTrendingUp, IconBrain, IconTarget,  IconHome } from '@tabler/icons-react';
...
export const WorkshopLandingPage = () => {
  ...
  const handleWorkshopInquiry = () => {
    const subject = encodeURIComponent('Workshop Anfrage: AI - Low Hanging Fruits');
      ...
      >
        ...
        <Section>
          <motion.div
            ...
            animate="visible"
          >
            <Grid cols={{ mobile: 1, tablet: 1, desktop: 2 }} spacing="xl" style={{ alignItems: 'center' }}>
              {/* Text Content */}
              <Stack gap="xl" ta={isMobile ? 'center' : 'left'}>
                ...
                <Text size="xl" c="var(--text-secondary)" style={{ lineHeight: 1.6 }}>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (contactSection) {
                        ...
                        const elementPosition = (contactSection as HTMLElement).offsetTop - headerHeight;
                        window.scrollTo({
                          top: elementPosition,
                          behavior: 'smooth'
                        });
              ...
              <Box style={{ display: 'flex', justifyContent: 'center', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                <Image
              ...
              >
                <Stack gap="md">
                  ...
                  <List spacing="sm" icon={
                    ...
                    {t.workshop.problem.points.map((point: string, index: number) => (
              ...
              >
                <Stack gap="md">
                  ...
                  <List spacing="sm" icon={
                    ...
                    {t.workshop.solution.points.map((point: string, index: number) => (
                      <List.Item key={index}>{point}</List.Item>
                    ))}
                  </List>
                </Stack>
              ...
              <Grid cols={{ mobile: 1, tablet: 1, desktop: 2 }} spacing="xl" style={{ alignItems: 'center' }}>
                ...
                <Stack gap="md">
                  ...
                  >
                    ...
                    >
                      <Box style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <ThemeIcon size="xl" style={{ background: 'linear-gradient(135deg, var(--primary-orange), var(--primary-red))' }}>
                          <IconClock size={28} />
                        </ThemeIcon>
                        <Box>
                          <Title order={4}>{t.workshop.details.duration.title}</Title>
                          <Text c="var(--text-primary)" fw={500}>{t.workshop.details.duration.value}</Text>
                          <Text size="sm" c="var(--text-secondary)" style={{ lineHeight: 1.4 }}>{t.workshop.details.duration.description}</Text>
                        </Box>
                      </Box>
                    </Card>
                  </motion.div>
                  ...
                  >
                    ...
                    >
                      <Box style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <Box>
                          <Title order={4}>{t.workshop.details.participants.title}</Title>
                          <Text c="var(--text-primary)" fw={500}>{t.workshop.details.participants.value}</Text>
                          <Text size="sm" c="var(--text-secondary)" style={{ lineHeight: 1.4 }}>{t.workshop.details.participants.description}</Text>
                        </Box>
                      </Box>
                    </Card>
                  </motion.div>
                  ...
                  >
                    ...
                    >
                      <Box style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        ...
                        <Box>
                          <Title order={4}>{t.workshop.details.outcome.title}</Title>
                          <Text c="var(--text-primary)" fw={500}>{t.workshop.details.outcome.value}</Text>
                          <Text size="sm" c="var(--text-secondary)" style={{ lineHeight: 1.4 }}>{t.workshop.details.outcome.description}</Text>
                        </Box>
                      </Box>
                    </Card>
                  </motion.div>
              ...
              <Grid cols={{ mobile: 1, tablet: 1, desktop: 2 }} spacing="xl">
                {t.workshop.agenda.items.map((item, index: number) => (
                  ...
                  >
                    ...
                    >
                      <Stack gap="md">
                        <Title order={3} c="var(--text-primary)">{item.title}</Title>
        ...
        <Section>
          ...
          >
            <Stack gap="xl">
              ...
              >
                <Stack gap="lg" align="center" style={{ height: '100%', justifyContent: 'space-between' }}>
                ...
                <Stack gap="lg" align="center" style={{ height: '100%', justifyContent: 'space-between' }}>
                  <Stack gap="lg" align="center">
                    ...
                    <Text c="var(--text-secondary)">
                      Entdecken Sie meine anderen Services, Projekte und Referenzen auf der Hauptseite.
                    </Text>
                  </Stack>
