...
describe("projects data", () => {
  ...
  describe("content validation", () => {
    it("should have common technology tags", () => {
      const allTags = projectsData.projects.flatMap((p) => [
        ...p.primary_tags,
        ...p.tags,
      ]);
    ...
    it("should have AI-related projects", () => {
      const aiProjects = projectsData.projects.filter(
        (p) =>
    ...
    it("should have cloud-related projects", () => {
      const cloudProjects = projectsData.projects.filter(
        (p) =>
          p.primary_tags.some((tag) =>
    ...
    it("should have web development projects", () => {
      const webProjects = projectsData.projects.filter((p) =>
        p.tags.some((tag) =>
          ["React", "Angular", "TypeScript", "JavaScript"].includes(tag)
        )
      );
  ...
  describe("specific project validation", () => {
    it("should contain expected high-profile projects", () => {
      ...
      const hasAIProjects = projectTitles.some((title) =>
        title.toLowerCase().includes("ai")
      );
      const hasDMGProjects = projectsData.projects.some((p) =>
        p.customer.includes("DMG")
      );
  ...
  describe("data integrity", () => {
    it("should not have duplicate projects", () => {
      const signatures = projectsData.projects.map(
        (p) => `${p.customer}-${p.title}`
      );
